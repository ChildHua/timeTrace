    ...
    <base-page>
        <template slot="title">
            <h3>Time Trace
                ...
                <svg @click="goHome" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
	 viewBox="0 0 59 59" class="home-icon" xml:space="preserve">
...
<rect x="48.5" y="11" style="fill:#ED8A19;" width="4" height="38"/>
        ...
        <div>
            <v-chart :forceFit="true" :height="height" :data="dayChartData" :scale="scale">
        ...
        computed:{
            dayChartData:function () {
                ...
                this.$store.state.timeTags.map(tag=>{
                    charSource.push({item:tag.name,id:tag.id,count:0})
                });
                charSource.push({item:'未标记',id:0,count:0});
                ...
                const dv = new DataSet.View().source(charSource);
                dv.transform({
                    type: 'percent',
                    field: 'count',
                    dimension: 'item',
